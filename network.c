...
double network_weights_input[Number_of_Layer-2][Number_of_Hidden_Neurons][Number_of_Hidden_Neurons];
...
void initial_network_weights() {
    ...
    for (int i = 0; i < (Number_of_Layer-1); i++) {
        for (int n = 0; n < Number_of_Hidden_Neurons; n++) {
                for (int x = 0; x < Number_of_Hidden_Neurons; x++) {
                    network_weights_input[i][n][x] = (float)rand() / RAND_MAX * 0.001;
                }
        }
    }
...
void hidden_layer(int layer, int neuron_number, double activated_neurons[Number_of_Layer-1][Number_of_Output_Neurons], int temp_board[8][8], int player) {
    ...
    if (!layer) {
        for (int x = 0; x < 8; x++) {
            for (int y = 0; y < 8; y++) {
                    sum += temp_board[x][y] * network_weights_input[layer][neuron_number][(x * 8) + y];
            }
        }
        sum += player * network_weights_input[layer][neuron_number][64];
    } else {
        for (int n = 0; n < Number_of_Hidden_Neurons; n++) {
            sum += activated_neurons[layer-1][n] * network_weights_input[layer][neuron_number][n];
        }
    }
